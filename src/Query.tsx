        ...
        <div className="mx-auto max-w-2xl pt-12 sm:pt-12 lg:pt-18">
          ...
          <div className="text-center">
            ...
            <p className="mt-2 text-left text-pretty text-base font-semibold text-gray-700 sm:text-lg">
              ...
              <br />アプリがクラッシュしているかどうかは、App Developer Console から見れるのですが、ゲームを作った際もクラッシュ率が0.0%でしたので、
              クラッシュしないアプリをつくるtipsを2つほど共有させていただきたいと思います。
              <br />1. 軽いフレームワークを使う技術選定をする。特にゲームエンジンを搭載しているフレームワークなどはよく落ちます。
              そもそもデフォルトで重いので、fpsを出すのに苦労します。ゲーム機器とは違って、
              高スペックなスマートフォンの人もいれば、骨董品レベルのものを未だに使い続けている人もいるわけです。
              <br />最近はiOSのバージョンが古い人、つまり昔のiPhoneを使い続けているためアップデートできない人は
              インストールがそもそもできないようになっています。
              <br />クラッシュしないためにCPUを積んだ最新機種のみ対応という流れは、それでいいのかという気がします。
              <br />例えばゲームエンジンを使わなくとも簡単なゲームはsprite機能を実装することでつくれますし、軽くするための工夫のしどころは色々あります。
              
              <br />2. 不要なモジュールやプラグインがたまっていて重くなっている。
              <br />このパターンは開発全般であるあるかと思います。
              アプリの場合はモジュールを読み込みすぎると、相性の問題でビルドが通らなかったり、クラッシュしたりします。
              いわゆるリファクタを定期的にする必要があります。
